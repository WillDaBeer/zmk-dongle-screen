 ...
 *
 * caps-word with a state flag the dongle screen can poll.
...
#define DT_DRV_COMPAT zmk_behavior_caps_word_ind
...
bool caps_word_ind_is_active(void) { return caps_word_ind_active_flag; }
...
struct behavior_caps_word_ind_config {
    zmk_mod_flags_t mods;
    uint8_t continuations_count;
    struct caps_word_continue_item continuations[];
};

struct behavior_caps_word_ind_data {
    bool active;
};

static void activate_caps_word(const struct device *dev) {
    struct behavior_caps_word_ind_data *data = dev->data;

    data->active = true;
    caps_word_ind_active_flag = true;
}
...
static void deactivate_caps_word(const struct device *dev) {
    ...
    data->active = false;
    caps_word_ind_active_flag = false;
}
...
static const struct behavior_driver_api behavior_caps_word_ind_driver_api = {
...
static int caps_word_ind_keycode_state_changed_listener(const zmk_event_t *eh);

ZMK_LISTENER(behavior_caps_word_ind, caps_word_ind_keycode_state_changed_listener);
ZMK_SUBSCRIPTION(behavior_caps_word_ind, zmk_keycode_state_changed);

#define GET_DEV(inst) DEVICE_DT_INST_GET(inst),
static const struct device *devs[] = {DT_INST_FOREACH_STATUS_OKAY(GET_DEV)};

static bool caps_word_is_caps_includelist(const struct behavior_caps_word_ind_config *config,
...
static void caps_word_enhance_usage(const struct behavior_caps_word_ind_config *config,
...
static int caps_word_ind_keycode_state_changed_listener(const zmk_event_t *eh) {
    ...
    for (int i = 0; i < ARRAY_SIZE(devs); i++) {
        ...
        const struct behavior_caps_word_ind_config *config = dev->config;
...
#define KP_INST(n)                                                                                 \
    static struct behavior_caps_word_ind_data behavior_caps_word_ind_data_##n = {.active = false}; \
    static const struct behavior_caps_word_ind_config behavior_caps_word_ind_config_##n = {        \
        .mods = DT_INST_PROP_OR(n, mods, MOD_LSFT),                                                \
        .continuations = {LISTIFY(DT_INST_PROP_LEN(n, continue_list), BREAK_ITEM, (, ), n)},       \
        .continuations_count = DT_INST_PROP_LEN(n, continue_list),                                 \
    };                                                                                             \
    BEHAVIOR_DT_INST_DEFINE(n, NULL, NULL, &behavior_caps_word_ind_data_##n,                       \
                            &behavior_caps_word_ind_config_##n, POST_KERNEL,                       \
                            CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &behavior_caps_word_ind_driver_api);
